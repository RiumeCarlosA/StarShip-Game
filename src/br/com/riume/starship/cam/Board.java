package br.com.riume.starship.cam;
...
import br.com.riume.starship.sprite.Alien;
import br.com.riume.starship.sprite.Bomb;
import br.com.riume.starship.sprite.Player;
import br.com.riume.starship.sprite.Shot;
...
public class Board extends JPanel {
	...
	private void update() {
		...
		if (deaths == Params.NUMBER_OF_ALIEN_TO_DISPLAY) {
			inGame = false;
			timer.stop();
			message = "You won!";
		}
		...
		if (shot.isVisible()) {
			...
			for (Alien alien : aliens) {
				...
				if (alien.isVisible() && shot.isVisible()) {
					if (shotX >= (alienX) && shotX <= (alienX + Params.ALIEN_WIDTH) && shotY >= (alienY)
		...
		for (Alien alien : aliens) {
			...
			if (x >= Params.BOARD_WIDTH - Params.BOARD_RIGHT && direction != -1) {
				direction = -1;
			...
			if (x <= Params.BOARD_LEFT && direction != 1) {
				direction = 1;
		...
		for (Alien alien : aliens) {
			...
			if (player.isVisible() && !bomb.isDestroyed()) {
				if (bombX >= (playerX) && bombX <= (playerX + Params.PLAYER_WIDTH) && bombY >= (playerY)
